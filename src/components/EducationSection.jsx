        ...
        {education.map((item, index) => (
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                {item.gpa ? (
                  <p className="mt-2 text-sm font-medium text-slate-600 dark:text-slate-300">
                    {item.gpa.startsWith("Percentage:") ? (
                      <>
                        <span className="font-semibold text-slate-900 dark:text-white">Percentage:</span>
                        <span className="font-semibold text-coral"> {item.gpa.replace("Percentage:", "").trim()}</span>
                      </>
                    ) : (
                      <>
              ...
              <span className="badge shrink-0">{item.duration}</span>
            </div>
            <p className="mt-3 text-sm font-medium text-coral sm:text-base">{item.institution}</p>
